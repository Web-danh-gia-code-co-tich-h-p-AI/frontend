...
const BlockXemDiem = () => {
    ...
    const handleSearch = (event) => {
        ...
        setCurrentPage(1);
        if (searchTerm) {
            const filteredUsers = users.filter(user => user.studentID.toString().includes(searchTerm));
            const hiddenIds = users.filter(user => !user.studentID.toString().includes(searchTerm)).map(user => user.studentID);
            ...
            const usersToShow = filteredUsers.concat(users.filter(user => hiddenIds.includes(user.studentID)));
    ...
    const handleSaveDetail = async () => {
        try {
            await axios.put(`${connectionString}/update-user/${selectedUser.studentID}`, selectedUser, {
                headers: {
                    'Authorization': basicAuthHeader,
                    "Content-Type": "application/json",
                    'Access-Control-Allow-Origin': '*',
            ...
            <div className='h-full -ml-4'>
                <div className='flex justify-between'>
                    <h2 className='flex pl-4 mt-3 mb-4 text-3xl text-zinc-700 text-[24px] laptop:text-[36px]'>
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-6 h-6 mr-3 scale-150 translate-y-2">
                    ...
                    <button 
                        ...
                        className={`mt-3 px-4 py-2 mr-3 mb-4 font-bold text-white bg-blue-500 rounded hover:bg-blue-700 ${isReloading ? 'opacity-50 cursor-not-allowed' : ''}`}
                        ...
                        {isReloading ? (
                            <div className="flex items-center">
                                <svg className="w-4 h-4 mr-2 text-white animate-spin" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                                    ...
                                    <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V2.5A1.5 1.5 0 0010.5 1h-3A1.5 1.5 0 006 2.5V4a8 8 0 018 8h2.5a1.5 1.5 0 001.5-1.5v-3a1.5 1.5 0 00-1.5-1.5H14"></path>
                                </svg>
                                Reloading...
                            </div>
                        ) : (
                ...
                <table className="w-full">
                    <thead className=''>
                        <tr className="bg-gray-100 text-zinc-600">
                            ...
                            <th className="hidden px-4 py-2 text-center laptop:table-cell">Đánh giá</th>
                            <th className="hidden px-4 py-2 text-center laptop:table-cell">Score AI</th>
                            <th className="hidden px-4 py-2 text-center laptop:table-cell">Note</th>
                            <th className="px-4 py-2 text-center">Actions</th>
                        </tr>
                    </thead>
                    <tbody>
                        {currentUsers.map(user => (
                            <tr key={user.studentID} className="border-b border-gray-300 text-zinc-800 ">
                            <td className="px-4 py-2 text-center ">{user.studentID}</td>
                            <td className="px-4 py-2 text-center ">
                                {user.score} 
                            </td>
                            <td className="hidden px-4 py-2 text-center laptop:table-cell">
                                <pre className='truncate max-w-32'>
                                    {user.evaluation} 
                                </pre>
                            </td>
                            <td className="hidden px-4 py-2 text-center laptop:table-cell">
                            ...
                            <td className="flex justify-between px-4 py-2 -mr-6 max-w-36">
                                <button onClick={() => handleDetail(user)} className="w-1/2 px-4 py-2 m-1 font-bold text-white bg-blue-500 rounded hover:bg-blue-700">
                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-6 h-6">
                                    <path strokeLinecap="round" strokeLinejoin="round" d="M19.5 14.25v-2.625a3.375 3.375 0 0 0-3.375-3.375h-1.5A1.125 1.125 0 0 1 13.5 7.125v-1.5a3.375 3.375 0 0 0-3.375-3.375H8.25m5.231 13.481L15 17.25m-4.5-15H5.625c-.621 0-1.125.504-1.125 1.125v16.5c0 .621.504 1.125 1.125 1.125h12.75c.621 0 1.125-.504 1.125-1.125V11.25a9 9 0 0 0-9-9Zm3.75 11.625a2.625 2.625 0 1 1-5.25 0 2.625 2.625 0 0 1 5.25 0Z" />
                                    </svg>
                                </button>
                                <button onClick={() => handleDelete(user.studentID)} className="w-1/2 px-4 py-2 m-1 text-sm font-bold text-white bg-red-500 rounded hover:bg-red-700">
                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-6 h-6">
                        ...
                        <div className="max-h-full p-8 bg-white rounded-lg text-zinc-700">
                            ...
                            <div className='max-h-full'>
                                ...
                                <div className="flex justify-end">
                                    <button onClick={handleCloseDetail} className="px-4 py-2 mr-2 text-gray-800 bg-gray-300 rounded-md hover:bg-gray-400">Đóng</button>
                                    <button onClick={handleSaveDetail} className="px-4 py-2 text-white bg-blue-500 rounded-md hover:bg-blue-600">Lưu</button>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    {users.length > 0 && (
                        <ul className="flex">
                            {Array.from({ length: Math.ceil(users.length / usersPerPage) }, (_, index) => (
                                <li key={index}>
                                    <button onClick={() => paginate(index + 1)} className="px-3 py-1 mb-5 mr-2 text-gray-700 bg-gray-200 rounded mt-9 hover:bg-gray-300 hover:-translate-y-1">{index + 1}</button>
                                </li>
                            ))}
                        </ul>
                    )}
