...
const labelClass = 'block text-zinc-800 text-sm font-medium dark:text-black-300 flex items-center'; // Thêm class 'flex items-center' để căn SVG và label cùng một hàng
const inputBaseClass = 'mt-1 mb-3 block w-full px-3 py-2 border border-zinc-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 dark:bg-white dark:border-zinc-600 dark:text-black dark:focus:border-indigo-500';
...
const FormNhapDiem = ({ generatedValues }) => {
  ...
  useEffect(() => {
    ...
    if (generatedValues && 'Score' in generatedValues && 'Note' in generatedValues) {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const response = await axios.post('https://bewbewbew-001-site1.ftempurl.com/api/demo/add-user', {
  ...
  return (
    <div className={containerClass}>
      <form className="space-y-4 " onSubmit={handleSubmit}>
        <FormField id="studentId" label="Mã sinh viên" type="number" placeholder="Nhập mã sinh viên" />
        <FormField id="evaluationScore" label="Điểm đánh giá" type="number" placeholder="Nhập điểm đánh giá trên thang 100" value={value} onChange={handleChange}/>
        <FormField type='textarea' id="teacherNotes" label="Ghi chú của giáo viên" placeholder="Nhập ghi chú" />
        {/* Truyền giá trị mặc định cho scoreAI và aiNotes */}
        <FormField id="scoreAI" label="Điểm AI" placeholder="Điểm AI..." value={scoreAI} />
        <FormField type='textarea' id="aiNotes" label="Ghi chú AI" placeholder="Ghi chú AI..." value={aiNotes} />
        <div className="w-full ">
          <button type="submit" className={buttonClass}>
              Gửi thông tin
              <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16 16" fill="currentColor" className="w-4 h-4 ml-3">
